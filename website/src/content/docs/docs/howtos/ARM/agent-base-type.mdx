...
- **Appliance** — the service owns and reports the agent configuration. Definition and agent
  properties are **read-only**, except `modelDeploymentRef`: the `definition` container stays
  writable so the client can select the underlying model deployment.
...
`modelDeploymentRef` is writable in both deployment models: even when the service owns the rest of
the appliance definition, the client selects the underlying model deployment. For this to work on
the wire, `AgentPropertiesAppliance` also leaves the `definition` container itself writable — a
read-only container would make every field inside it unsettable, including `modelDeploymentRef`.
...
```typespec
@azureBaseType(#{ baseType: "Agent", version: "2026-04-01" })
