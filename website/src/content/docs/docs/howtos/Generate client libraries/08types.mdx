...
## Using `@alternateType` to Reference External Types

The `@alternateType` decorator allows you to override the emitted type for a model, union, or enum in generated client libraries. With recent updates, you can now reference types defined in external namespaces or imported libraries.

### Example: Referencing an External Type

Suppose you want your generated client library to use a type from an external package instead of the default emitted type.

```typespec
@alternateType(
  {
    fullyQualifiedName: "externallib.ExternalModel",
    package: "externallib",
  },
  "python"
)
...
In this example, instead of generating `MyModel` in the Python sdk, the emitter will reference `ExternalModel` from `externallib` instead.

### Use Cases

- Integrating with shared types from other services or libraries.
- Ensuring consistency across multiple SDKs by reusing common models.
- Reducing duplication when types are already defined elsewhere.
...
:::caution
The `@clientDefaultValue` decorator is legacy functionality that should **only** be used to mitigate SDK breaking changes for brownfield services. **New services should NOT use this decorator.** Instead, use standard TypeSpec patterns for default values.
:::

The `@Azure.ClientGenerator.Core.Legacy.clientDefaultValue` decorator allows you to set a client-level default value for a model property or operation parameter. This is primarily intended for maintaining backward compatibility with existing SDK users who may rely on default values that were previously generated from Swagger definitions.

### Model Property Default Values

Use `@clientDefaultValue` on model properties when you need to specify a default value that will be used in the generated SDK code.
...
```python
# Not supported yet in python and track status in https://github.com/microsoft/typespec/issues/9198
```
...
You can also scope the default value to specific languages using the `scope` parameter:
...
```typespec
model Config {
  // Apply default only for Python
  @Azure.ClientGenerator.Core.Legacy.clientDefaultValue(false, "python")
  enableCache?: boolean;

  // Apply default for all languages except Java and Python
  @Azure.ClientGenerator.Core.Legacy.clientDefaultValue(100, "!(java, python)")
  maxRetries?: int32;
}
```
...
The default value you pass to `@clientDefaultValue` must have the same type as the property or parameter it is applied to. TCGC emits the `client-default-value-type-mismatch` warning when the types disagree — for example, a string default (`"10"`) on a numeric property, or a numeric default (`123`) on a string property. Numeric subtypes such as `float32` and `int64` are treated as numeric, so a numeric default is valid for any of them.

If you use `@alternateType` to change the client-facing type of the property, the default value is validated against the alternate type instead. This lets you keep a string default on a property whose wire type is numeric:

```typespec
@Azure.ClientGenerator.Core.Legacy.clientDefaultValue("10")
@Azure.ClientGenerator.Core.alternateType(string)
@query pageSize?: int32; // no warning — the default matches the alternate type
```

If a mismatch is intentional, suppress the warning with `#suppress "@azure-tools/typespec-client-generator-core/client-default-value-type-mismatch"`. Note that the mismatched default value is still applied to the generated SDKs when suppressed.
...
```ts
// JS CodeGen for both legacy and non legacy
...
// JS CodeGen for non-legacy clients
export interface AnimalProperty {
  category: string;
  value: any;
}
...
type Animal struct {
  Name *string
  Kind *string
  AdditionalProperties map[string]AnimalProperty
}
...
```ts
// JS CodeGen for legacy and non-legacy clients
...
namespace TestService
{
  public partial class Foo
  {
    public string BasicNullableProperty { get; }
    public Bar ModelNullableProperty { get; }
    public LR? EnumNullableProperty { get; }
  }
...
```java
// All Java reference type is nullable.
public final class Foo implements JsonSerializable<Foo> {
    public String getBasicNullableProperty();
    public Bar getModelNullableProperty();
    public LR getEnumNullableProperty();
}
...
```go
type Foo struct {
  BasicNullableProperty *string
  ModelNullableProperty *Bar
  EnumNullableProperty  *LR
}
...
```typescript
// This is for legacy pattern, for new services/APIs, it's not recommended.
export interface Foo {
  name: string;
}
export function _fooPropSerializer(item: Foo): any {
  return { name: item["name"] };
}

export function _fooPropDeserializer(item: any) {
  return {
    name: item["name"],
  };
}
...
type WidgetHorizontal string

const (
  WidgetHorizontalLeft WidgetHorizontal = "left"
  WidgetHorizontalRight WidgetHorizontal = "right"
)
...
export function testSerializer(item: Test): any {
  return { prop: item["prop"].toISOString() };
}

export function testDeserializer(item: any): Test {
  return {
    prop: new Date(item["prop"]),
  };
}
```
...
```csharp
// The property is exposed as `long`; serialization writes it as a JSON string.
namespace TestService
{
  public partial class Test
  {
    public long Prop { get; }
  }
}
...
// Custom MarshalJSON converts int64 to a JSON string.
func (t Test) MarshalJSON() ([]byte, error) {
    objectMap := make(map[string]any)
    populate(objectMap, "prop", to.Ptr(strconv.FormatInt(*t.Prop, 10)))
