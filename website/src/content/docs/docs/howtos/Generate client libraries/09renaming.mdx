...
#### Handling reserved-word operations (TypeScript)

In the TypeScript emitter, operations whose TypeSpec name is a JavaScript reserved word (e.g. `delete`) are automatically renamed to avoid conflicts. When the operation belongs to an **operation group**, the emitter suffixes the **singularized** operation group name:
...
Generated TypeScript (data-plane):

```typescript
export interface ConversationsOperations {
  // Renamed from `delete` to `deleteConversation` (singularized group name)
  deleteConversation: (conversationName: string, options?: ConversationsDeleteOptionalParams) => Promise<void>;
}
```

If the operation has **no operation group**, it falls back to a `$`-prefixed internal name with a `@fixme` doc comment guiding you to add a `@clientName` override.

**Management-plane** services are not affected — `delete` is emitted as-is, matching long-standing HLC behavior.

**Escape hatch via `@clientName`**: If a library already shipped with the guarded `delete` name and you need to preserve it, apply `@clientName` explicitly. The emitter then honors that name and skips the automatic rename:
