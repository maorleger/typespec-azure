...
| `value`   | `unknown`          | The value for the option. Can be a string, boolean, number, or a TypeSpec model reference. |
...
The decorator supports string, boolean, numeric values, and TypeSpec model references:
...
// Model reference: pass a TypeSpec model as the option value
...
TCGC provides the `getClientOptions` helper function to easily look up client option values on any decorated SDK type.
...
// Get a specific client option value from a model
const sdkModel = context.sdkPackage.models.find((m) => m.name === "MyModel");
const value = getClientOptions(sdkModel, "enableFeatureFoo");
// value is the option value (e.g., true), or undefined if not set
```

The `getClientOptions` function takes a decorated SDK type and an option key, and returns the option value (`unknown`) or `undefined` if the option is not set:

```typescript
function getClientOptions<T extends DecoratedType>(type: T, key: string): unknown;
```
...
```typescript
// Models
const modelValue = getClientOptions(sdkModel, "myOption");

// Enums
const enumValue = getClientOptions(sdkEnum, "myOption");

// Operations/Methods
const methodValue = getClientOptions(sdkMethod, "myOption");

// Properties
const propertyValue = getClientOptions(sdkProperty, "myOption");

// Clients
const clientValue = getClientOptions(sdkClient, "myOption");
```
