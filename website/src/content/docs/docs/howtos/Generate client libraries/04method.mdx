...
:::note
`@convenientAPI` and `@protocolAPI` are only meaningful for Java and C# emitters. Always include a language scope of `"java"` or `"csharp"` to avoid a [`decorator-requires-scope`](https://azure.github.io/typespec-azure/docs/libraries/typespec-client-generator-core/diagnostics/decorator-requires-scope/) warning.
:::
...
@@convenientAPI(PetStoreNamespace.GetModel, false, "java");
@@convenientAPI(PetStoreNamespace.GetModel, false, "csharp");
```
...
func (client *Client) Post(ctx context.Context, body User, options *ClientPostOptions) (ClientPostResponse, error)
```
...
func (client *Client) ScheduleRepairs(ctx context.Context, widgetName string, repairInfo RepairInfo, options *ClientScheduleRepairsOptions) (ClientScheduleRepairsResponse, error)
...
func (client *AnalyzeClient) Analyze(ctx context.Context, mode AnalyzeRequestMode, imageParam io.ReadSeekCloser, options *AnalyzeClientAnalyzeOptions) (AnalyzeClientAnalyzeResponse, error)

func (client *AnalyzeClient) Completions(ctx context.Context, input CompletionInput, options *AnalyzeClientCompletionsOptions) (AnalyzeClientCompletionsResponse, error)
...
func (client *Client) A(ctx context.Context, x int32, options *ClientAOptions) (ClientAResponse, error)

func (client *Client) B(ctx context.Context, x string, options *ClientBOptions) (ClientBResponse, error)
```
...
func (client *Client) Get(ctx context.Context, options *ClientGetOptions) (*Response, error)
...
:::caution
`@clientLocation` cannot move multiple parameters that share the same name but have **different types** to the same client. This commonly happens when `@clientLocation` is applied to a templated parameter that is instantiated with different types across operations. Doing so emits a `client-location-conflict` warning and produces a broken client where the parameter collapses to a single type. Move the parameter on each operation instead, so it keeps a consistent type on the client.
:::

## Customizing method signatures with `@override`

The `@override` decorator allows you to customize method signatures in generated client SDKs. This is useful when you need to:

- Make optional parameters required for certain languages
- Add language-specific convenience parameters
- Change the return type for better developer experience
- Group parameters into option bags
- Remove parameters that shouldn't be exposed in the client

### Basic usage

The `@override` decorator takes a target operation and a custom operation that defines the new signature:

```typespec
@@override(OriginalOperation, CustomOperation);
@@override(OriginalOperation, CustomOperation, "python"); // Language-specific
```

When applied, the method parameters come from the override operation while the underlying HTTP operation remains unchanged. TCGC maps method parameters to operation parameters based on name matching.

:::caution
The override operation must preserve every parameter that is realized as a path parameter in the original operation's HTTP route, keeping its `@path` decorator. Dropping such a parameter — or redeclaring it without `@path` — reports an `override-parameters-mismatch` error, because the underlying route still requires that value. To relocate a parameter instead of dropping it, use [`@clientLocation`](#using-clientlocation-to-control-parameter-placement).
:::

### Simple parameter customization
...
// Define operation with required parameter
op getSecretsCustom(@query maxResults: int32): void;

// Apply the override
@@override(MyService.getSecrets, getSecretsCustom);
```
...
```python
# maxResults is now required instead of optional
def get_secrets(self, max_results: int) -> None:
    ...
```
...
```csharp
// maxResults is now required
public virtual Response GetSecrets(int maxResults, CancellationToken cancellationToken = default)
```
...
```typescript
// maxResults is now required
export async function getSecrets(maxResults: number): Promise<void>;
```
...
```java
// maxResults is now required
public void getSecrets(int maxResults);
```
...
For composable customizations, TCGC provides experimental transformation functions that enable chainable modifications without manually defining custom operations.

:::caution
These functions are experimental and require suppressing the `experimental-feature` diagnostic.
:::
...
When used with `@@override`, only optional parameters can be removed. Attempting to remove a required parameter will result in an error.
...
#suppress "experimental-feature" "using removeParameter"
...
```typescript
// maxResults parameter is removed
export async function getSecrets(): Promise<void>;
```
...
```typespec title="client.tsp"
model ConvenienceParams {
  @header("X-Request-Id") requestId?: string;
}

#suppress "experimental-feature" "using addParameter"
@@override(uploadFile, addParameter(uploadFile, ConvenienceParams.requestId));
```
...
```python
# requestId parameter is added
def upload_file(self, content: bytes, *, request_id: Optional[str] = None) -> None:
    ...
```
...
```csharp
// requestId parameter is added
public virtual Response UploadFile(BinaryData content, string requestId = null, CancellationToken cancellationToken = default)
```
...
```typescript
// requestId parameter is added in options
export interface UploadFileOptions {
  requestId?: string;
}
export async function uploadFile(content: Uint8Array, options?: UploadFileOptions): Promise<void>;
```
...
```java
// requestId parameter is added
public void uploadFile(byte[] content, String requestId);
```
...
Reorder parameters of an operation according to a specified order:
...
```typespec title="client.tsp"
// Reorder parameters: put 'c' first, then 'a', then 'b'
...
def my_op(self, c: bool, a: str, b: int) -> None:
    ...
```
...
```csharp
// Parameters are now in the order: c, a, b
public virtual Response MyOp(bool c, string a, int b, CancellationToken cancellationToken = default)
```
...
```typescript
// Parameters are now in the order: c, a, b
export async function myOp(c: boolean, a: string, b: number): Promise<void>;
```
...
```java
// Parameters are now in the order: c, a, b
public void myOp(boolean c, String a, int b);
```
...
Transformation functions can be chained together. Each function returns a new operation that can be passed to the next transformation:
...
op search(@query query?: string, @query limit?: int32): string;
...
// Chain: make query required -> make limit required -> reorder parameters
#suppress "experimental-feature" "using replaceParameter"
alias step1 = replaceParameter(search, "query", RequiredParams.query);
#suppress "experimental-feature" "using replaceParameter"
alias step2 = replaceParameter(step1, "limit", RequiredParams.limit);
#suppress "experimental-feature" "using reorderParameters"
@@override(search, reorderParameters(step2, #["limit", "query"]));
```
...
```python
# Both query and limit are now required, and limit comes before query
def search(self, limit: int, query: str) -> str:
    ...
```
...
```csharp
// Both parameters required, limit comes before query
public virtual Response<string> Search(int limit, string query, CancellationToken cancellationToken = default)
```
...
```typescript
// Both parameters required, limit comes before query
export async function search(limit: number, query: string): Promise<string>;
```
...
```java
// Both parameters required, limit comes before query
public String search(int limit, String query);
```
...
Use the `scope` parameter to apply customizations only to specific language emitters:

```typespec
// Only apply to Python - maxResults will be required in Python but remain optional in other languages
#suppress "experimental-feature" "using replaceParameter"
@@override(
  getSecrets,
  replaceParameter(getSecrets, "maxResults", RequiredMaxResults.maxResults),
  "python"
);
```
...
## HEAD Operations with Boolean Response (`@responseAsBool`)

For HEAD operations, you can use `@responseAsBool` to model the response as a boolean value. A successful response (2xx) returns `true`, and a 404 response returns `false` instead of throwing an error.
...
@head
@route("/widgets/{name}")
op exists(@path name: string): void;
```
...
- The method returns a `boolean` value — this is a purely client-side concept computed from the HTTP status code
- A successful response (2xx) means the method returns `true`
- A 404 response means the method returns `false` (instead of throwing an error)
- The HTTP-level response has no body type; the boolean is derived entirely from the status code
- If the operation has no explicit 404 response, one is automatically added

:::note
This decorator can only be applied to HEAD operations. Applying it to operations with other HTTP verbs will result in an error.
:::
