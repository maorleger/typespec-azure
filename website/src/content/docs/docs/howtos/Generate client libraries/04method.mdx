...
> **NOTE:** `@convenientAPI` and `@protocolAPI` are only meaningful for Java and C# emitters. Always scope them to `"java"` and/or `"csharp"` to avoid a [`decorator-requires-scope`](../../libraries/typespec-client-generator-core/reference/decorators#@Azure.ClientGenerator.Core.convenientAPI) validation warning.
...
@@convenientAPI(PetStoreNamespace.GetModel, false, "java, csharp");
```
