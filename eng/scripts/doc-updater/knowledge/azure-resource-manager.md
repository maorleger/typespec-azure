...
- `@baseTypeOptional(isPresent, isAppliance)` (private decorator) controls base-type property visibility (invisible when not present; read-only when appliance). `AgentDefinitionAppliance.modelDeploymentRef` passes `isAppliance: false` on purpose so it stays writable in both deployment models.
